...
public class StartDoorBlock extends BlockDoor implements ITileEntityProvider {
	...
	public StartDoorBlock() {
		super(Material.iron);
		this.setBlockName("start_door");
		this.setBlockTextureName(CCLMaze.MODID + ":start_door");
	...
	public void registerBlockIcons(IIconRegister register) {
		...
		this.icons1[0] = register.registerIcon(CCLMaze.MODID + ":start_door_upper");
		this.icons2[0] = register.registerIcon(CCLMaze.MODID + ":start_door_lower");
	...
	public Item getItem(World worldIn, int x, int y, int z) {
		return MazeItems.startDoorItem;
	}

	@Override
	public Item getItemDropped(int meta, Random rand, int fortune) {
		return (meta & 8) != 0 ? null : MazeItems.startDoorItem;
	}
	...
	public int colorMultiplier(IBlockAccess worldIn, int x, int y, int z) {
		...
		return DoorColourLookup.colours.get(c);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
		if (world.isRemote) {
			player.openGui(CCLMaze.instance, MazeGuiHandler.START_DOOR_GUI_SCREEN, world, x, y, z);
			Configuration playerConfig = MazePlayerConfigLoader.get(player.getDisplayName());
			if (playerConfig != null) {
				double dy = 0.0;
				if ((world.getBlockMetadata(x, y, z) & 8) != 0) {
					dy = 1.0;
				}
				playerConfig.get("position", "x", MazeConfigHandler.TeleportX + 0.5f).set(x + 0.5f);
				playerConfig.get("position", "y", MazeConfigHandler.TeleportY).set(y - dy);
				playerConfig.get("position", "z", MazeConfigHandler.TeleportZ + 0.5f).set(z + 0.5f);
				playerConfig.save();
			}
		}
		return true;
